...
Lightweight recovery using CLAUDE.md (auto-loaded at startup).
...
**Note**: After `/clear`, your role instructions (`instructions/ashigaru.md`) are lost. However, `shutsujin_departure.sh` loaded them at deployment, and CLAUDE.md contains enough context to operate. If critical role info is needed, re-read `instructions/ashigaru.md` manually (rare case — most tasks don't need it).
